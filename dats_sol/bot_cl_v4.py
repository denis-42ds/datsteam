...
BUG 1 — "Продолжаем ЧУЖУЮ стройку"
    API /api/arena `construction` отдаёт ВСЕ видимые стройки, включая чужие.
    Бот видел соседнюю стройку врага, принимал её за свою и "продолжал" её.
    Прогресс в поле `progress` был прогрессом врага → убывал или прыгал.
    Fix: не полагаемся на сервер для выбора цели стройки.
         Держим bot-side переменную `escape_target` — единственная ячейка,
         в которую мы строим эскейп. Сбрасываем только при смерти/смене ЦУ.

BUG 2 — Пропуск каждого второго хода
    Бот спал `next_turn_in - 0.25` секунды. При next_turn_in ≈ 1с это 0.75с сна.
    После сна: GET arena (~100ms) + обработка + POST command (~100ms) = ~1.0с суммарно.
    Итого: мы едва успеваем на текущий ход, зато следующий почти всегда пропускается.
    При пропущенном ходе стройка деградирует (DS=8..10). Net за 2 хода ≈ CS - DS ≤ 0.
    Fix: sleep(max(0.02, next_turn_in - 0.05)) — просыпаемся за 50ms до нового хода.
         Плюс tight retry-loop если вдруг next_turn_in > 1.5 (поймали старый стейт).

PATCH-изменения:
    - Клетки деградируют через 20 ходов (было 80), ячейки освобождаются быстрее.
    - Бобры теперь 20x ценности клетки → самый прибыльный источник очков.
    - Критерий победы 2: меньше потерянных ЦУ (не плантаций).

Приоритет апгрейдов изменён:
    repair_power первый → CS 5→8 (стройка быстрее, переживаем пропуск хода)
    beaver_damage_mitigation → урон бобров 15→5 (при CS=8 стройка выживает)
    decay_mitigation → DS 10→4 (ещё больше запаса)
    settlement_limit → расширяем лимит когда выживаемость уже обеспечена
"""
...
BEAVER_MULTIPLIER = 20   # [PATCH]

CU_BUILD_NEIGHBOR_PCT = 35   # start building escape at this % cell progress
...
# [FIX] Upgrade order: repair_power + beaver_damage_mitigation first
# so constructions can survive before we expand with settlement_limit.
UPGRADE_PRIORITY = [
    "repair_power",              # CS 5→8 (3 upgrades): construction survives 1 missed turn
    "beaver_damage_mitigation",  # beaver 15→5 (5 upgrades): construction survives beaver lair
    "decay_mitigation",          # DS 10→4 (3 upgrades): wider margin for missed turns
    "settlement_limit",          # expand plantation cap once we can actually build
    "max_hp",
...
def get_frontier(connected, occupied, mountains, ar, map_w, map_h, beaver_positions):
    ...
    # Deterministic sort: score DESC, then coord ASC (tiebreak)
...
def main_loop():
    ...
    # [FIX] Bot-side escape state — NOT derived from server `constructions` API
    # (which can include enemy constructions and mislead us).
    escape_target : tuple | None = None   # the ONE adjacent cell we're building at
    last_cu_pos   : tuple | None = None   # to detect ЦУ death/respawn

    logging.info("DatsSol bot v4 started.")
    ...
    while True:
        try:
            ...
            if turn == last_turn:
                # Same turn — retry quickly (shouldn't happen often with tight timing)
                time.sleep(0.03)
                continue
            last_turn = turn
            ...
            # NOTE: state["construction"] intentionally NOT used for escape targeting
            #       (may contain enemy constructions). Used only for display.
            ...
            # ──────────────────────────────────────────────────────────────
            # [FIX] ESCAPE TARGET STATE MACHINE
            # ----------------------------------------------------------
            # We keep `escape_target` across turns. We NEVER read the
            # server's `construction` field to determine our escape target —
            # the server mixes our constructions with enemy constructions.
            # ──────────────────────────────────────────────────────────────

            # 1. Detect ЦУ death / respawn (position changed)
            ...
            if escape_target is None and cu_cell_prog >= CU_BUILD_NEIGHBOR_PCT:
                for nb in sorted(neighbors4(*main_pos), key=lambda p: (-cell_value(*p), p)):
                    ...
                    if nb in p_map and nb not in connected:
                        continue   # isolated plantation there — can't use for relocation
                    if not (0 <= nx < map_w and 0 <= ny < map_h):
                        continue
                    if nb not in p_map:   # free cell (or our construction in progress)
                        escape_target = nb
                        logging.warning(f"  🔑 New escape target: {nb} (ЦУ={cu_cell_prog:.0f}%)")
                        break
                    # nb is in p_map AND in connected → perfect, can relocate right now
                    if nb in connected:
                        escape_target = nb
                        escape_ready  = True
                        break
            ...
            if escape_target is None and cu_cell_prog >= CU_BUILD_NEIGHBOR_PCT:
                logging.warning(f"  🚨 No valid escape target adjacent to ЦУ! All 4 neighbors occupied.")
            ...
            # P0-A: Relocate ЦУ if target is ready and cell is dangerous
            if escape_ready and (cu_cell_prog >= CU_RELOCATE_PCT or main_p["hp"] <= CU_PANIC_HP):
                ...
                escape_target = None   # will pick a new target next turn from new ЦУ pos

            # P0-B: Build escape target — EVERY TURN while the cell is dangerous
            elif escape_target is not None and not escape_ready:
                # Best builder = closest connected plantation (usually the ЦУ itself)
                ...
                for builder in builders_by_dist:
                    ...
                    if cb.try_assign(bpos, escape_target, "escape-build"):
                        prog_str = ""
                        # Show progress from server for info (might be enemy's, but informative)
            ...
            # ══════════════════════════════════════════════════════════════
            # PRIORITY 4 — [PATCH 20x] Attack beaver lairs
            # Killing one lair = 20 × cell_value = up to 30k pts
            # ══════════════════════════════════════════════════════════════
            ...
            #   [FIX] Only fires for cells in `connected` (own plantations),
            #   only uses neighbors4 (4-directional), picks deterministically.
            # ══════════════════════════════════════════════════════════════
            for ppos, cell_data in cells.items():
                ...
                # Only 4-directional neighbors (must be adjacent for chain connection)
                adj_free = sorted(
            ...
            # ══════════════════════════════════════════════════════════════
            # PRIORITY 6 — Expand toward enhanced cells / beavers
            # ══════════════════════════════════════════════════════════════
            ...
            # ══════════════════════════════════════════════════════════════
            # PRIORITY 7 — Sabotage critically-low-HP enemies
            # ══════════════════════════════════════════════════════════════
            ...
            # ── [FIX] Tight timing: wake up ~50ms before next turn ────────
            # Previous: sleep(next_turn_in - 0.25) caused missing every other turn.
            # New: sleep(next_turn_in - 0.05) — leaves 50ms for fetch+process+submit.
            # The whole GET+process+POST cycle typically takes 150-300ms, so we
            # submit 0-200ms into the new turn, well within the 1-second window.
            time.sleep(max(0.02, next_turn_in - 0.05))

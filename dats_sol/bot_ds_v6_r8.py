...
===============
Root-cause fixes:
- ЦУ dies because terraformation reaches 100% before escape construction finishes.
  Solution: build **TWO** escape targets simultaneously with all available builders.
  Relocate immediately when ANY target becomes connected (no waiting for 75%).
- Turn skipping due to imprecise sleep: now sleep exactly `next_turn_in - 0.15` sec.
- Upgrade priority: repair_power → beaver_damage_mitigation → decay_mitigation.
- Aggressively attack beavers when enough plantations exist.
"""
...
def main_loop():
    ...
    logging.info("DatsSol bot v10 started.")
    ...
    while True:
        ...
        try:
            ...
            # ── ЦУ SURVIVAL: immediate relocate if any escape target is ready ──
            ready_target = next((t for t in escape_targets if t in connected), None)
            if ready_target:
                relocate_cmd = [list(main_pos), list(ready_target)]
                logging.warning(f"  🚨 RELOCATE ЦУ {main_pos}→{ready_target} (target ready)")
                escape_targets = []
            else:
                # Build all active escape targets with all available builders (up to 5 per target)
            ...
            # PRIORITY 4 — Attack beaver lairs (aggressive if enough plantations)
            if len(connected) >= 2:

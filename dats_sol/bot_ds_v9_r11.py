...
===============
Critical fixes:
- Single escape target (best adjacent cell) built with ALL available builders.
- Removed spin-wait to reduce turn skipping; use precise sleep only.
- Log construction progress vs terraformation progress for debugging.
- Immediate relocation if any adjacent plantation exists.
"""
...
def main_loop():
    ...
    logging.info("DatsSol bot v13 started.")
    ...
    while True:
        ...
        try:
            ...
            # PRIORITY 6 — Expand (only if we have spare builders after escape)
            if len(connected) >= 2 and escape_target is not None and len(cb.commands) < len(connected_plants):
            ...
            # ── Precise sleep only, no spin ─────────────────────────────
            elapsed = time.time() - loop_start
            sleep_for = max(0.0, next_turn_in - elapsed - 0.15)
            if sleep_for > 0.001:

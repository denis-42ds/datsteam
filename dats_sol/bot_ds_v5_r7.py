...
- Fixed turn skipping by precise sleep based on next_turn_in and elapsed time.
- Removed spin-wait that caused extra GET requests and delays.
- Use all available connected plantations to build escape targets (up to 5).
- Lowered relocate threshold to 75% for safety.
- Added retry on "command already submitted" error.
- Log elapsed time per loop for debugging.
"""
...
CU_RELOCATE_PCT       = 75   # relocate earlier to avoid death by terraforming
CU_PANIC_HP           = 15
...
def main_loop():
    ...
    logging.info("DatsSol bot v9 started.")
    ...
    while True:
        ...
        try:
            ...
            if ready_target and (cu_cell_prog >= CU_RELOCATE_PCT or main_p["hp"] <= CU_PANIC_HP):
                ...
                for target in escape_targets:
                    if target in connected:
                        continue
                    # Use all available connected plants as builders (up to 5)
                    builders = sorted(
                        connected_plants,
            ...
            if not cmds and not upgrade_to_buy and not relocate_cmd:
                ...
                if result:
                    ...
                    if errors:
                        ...
                        # If "command already submitted", we just continue
                    else:
                        if upgrade_to_buy:
            ...
            sleep_for = max(0.0, next_turn_in - elapsed - 0.05)
            if sleep_for > 0.001:

...
- Fixed turn skipping: sleep based on next_turn_in - 0.08 (constant) and then spin-wait until new turn.
- Use all available builders for escape target (up to 3 to avoid relay overuse).
- Start building escape on TWO adjacent cells simultaneously if possible (safety).
- Immediately expand after successful relocation.
- Log elapsed time and turn changes for monitoring.
"""
...
CU_BUILD_NEIGHBOR_PCT = 0    # start building escape immediately after respawn
CU_RELOCATE_PCT       = 80   # relocate when cell reaches this %
CU_PANIC_HP           = 15
...
def main_loop():
    ...
    logging.info("DatsSol bot v8 started.")
    ...
    while True:
        ...
        try:
            ...
            if main_pos != last_cu_pos:
                ...
                last_upgrade_turn = -1   # reset upgrade cooldown after respawn

            # Remove invalid targets (blocked, enemy, or already built)
            escape_targets = [t for t in escape_targets if t not in mountains and t not in enemy_pos_set and t not in connected]

            # Add new targets if needed (up to 2)
            if len(escape_targets) < 2:
                # Get all free adjacent cells
                free_adj = []
                for nb in neighbors4(*main_pos):
                    ...
                    if nb not in p_map:
                        free_adj.append(nb)
                    elif nb in connected:
                        # Already built - can be used for relocation
                        pass
                # Sort by value
            ...
            # Check if any target is ready for relocation (connected)
            ready_target = next((t for t in escape_targets if t in connected), None)
            if ready_target and (cu_cell_prog >= CU_RELOCATE_PCT or main_p["hp"] <= CU_PANIC_HP):
                relocate_cmd  = [list(main_pos), list(ready_target)]
                logging.warning(
                    f"  🚨 RELOCATE ЦУ {main_pos}→{ready_target} "
                    f"(cell={cu_cell_prog:.0f}%, hp={main_p['hp']})"
                )
                # After relocation, clear escape targets (they will be reset if ЦУ moves)
                escape_targets = []
            else:
                # Build all active escape targets
                for target in escape_targets:
                    if target in connected:
                        continue  # already built
                    # Use up to 3 closest builders
                    builders = sorted(
                        connected_plants,
                    ...
                    if assigned_any:
                        ...
                        if target in constr_map:
                            prog_str = f" server_prog={constr_map[target]}hp"
                        logging.warning(
            ...
            # PRIORITY 4 — Attack beaver lairs (if we have enough plantations)
            if len(connected) >= 3:
            ...
            # PRIORITY 7 — Sabotage (if we have spare capacity)
            if len(connected) >= 5:
            ...
            # ── SEND ────────────────────────────────────────────────────
            cmds = cb.commands
            # Force escape build if nothing else and we have targets
            if not cmds and escape_targets:
                for target in escape_targets:
                    if target not in connected:
                        if not cb.try_assign(main_pos, target, "force-escape"):
            ...
            # ── Timing: sleep until shortly before next turn, then spin ──
            elapsed = time.time() - loop_start
            sleep_for = max(0.0, next_turn_in - 0.08)
            if sleep_for > 0.001:

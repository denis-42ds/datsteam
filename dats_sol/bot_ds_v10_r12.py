...
DatsSol bot v14
===============
Focus: Ensure escape plantation finishes before terraformation kills ЦУ.
- Hybrid sleep + spin for precise turn sync.
- Build escape target with ALL builders every turn.
- No distractions (beavers, repair unless urgent) until 2+ plantations.
- Immediate relocate if adjacent connected plantation exists.
"""
...
URGENT_HP_THRESHOLD = 10   # only repair if very low, to avoid distraction
...
def main_loop():
    ...
    escape_target = None          # single best adjacent cell to build
    last_cu_pos   = None
    last_upgrade_turn = -1

    logging.info("DatsSol bot v14 started.")
    ...
    while True:
        ...
        try:
            ...
            # Validate current escape target
            if escape_target is not None:
                if escape_target in mountains or escape_target in enemy_pos_set or escape_target in connected:
                    escape_target = None

            # Pick best adjacent cell if needed
            if escape_target is None:
                ...
                for nb in neighbors4(*main_pos):
                    ...
                    if nb not in p_map:
                        free_adj.append(nb)
                if free_adj:
                    escape_target = max(free_adj, key=lambda p: cell_value(*p))
                    logging.warning(f"  🔑 New escape target: {escape_target} (ЦУ={cu_cell_prog:.0f}%)")
            ...
            if adjacent_plantations:
                ...
                # Check if escape target is ready (connected)
                if escape_target in connected:
                    relocate_cmd = [list(main_pos), list(escape_target)]
                    logging.warning(f"  🚨 RELOCATE ЦУ {main_pos}→{escape_target} (escape target ready)")
                    escape_target = None

            # If not relocating, build escape target with ALL builders
            if not relocate_cmd and escape_target is not None:
                ...
                for builder in builders:
                    if cb.try_assign(tuple(builder["position"]), escape_target, "escape-build"):
                        assigned += 1
                if assigned > 0:
                    prog_str = f" server_prog={constr_map[escape_target]}hp" if escape_target in constr_map else ""
                    logging.warning(
                        f"  🔨 Build escape {escape_target}{prog_str} "
                        f"(ЦУ={cu_cell_prog:.0f}%, need {build_turns_needed} turns, {assigned} builders)"
                    )

            # Only do other actions if we have at least 2 plantations (safer)
            if len(connected) >= 2:
                ...
                # PRIORITY 3 — Attack beaver lairs

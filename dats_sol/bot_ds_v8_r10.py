...
===============
Critical fixes:
- Build escape targets with ALL available connected plantations (max builders).
- Start building immediately (0% progress) after respawn detection.
- Immediate relocation if ANY adjacent connected plantation exists.
- Reduce turn skipping by precise sleep + retry.
- Attack beavers aggressively when we have at least 2 plantations.
- Log construction progress for debugging.
"""
...
def main_loop():
    ...
    logging.info("DatsSol bot v12 started.")
    ...
    while True:
        ...
        try:
            ...
            # If not relocating, build escape targets with ALL builders
            if not relocate_cmd:
                ...
                # Also build one extra expansion target
            ...
            # PRIORITY 4 — Attack beaver lairs (aggressive)
            if len(connected) >= 2:
            ...
            sleep_for = max(0.0, next_turn_in - elapsed - 0.25)
            if sleep_for > 0.001:
                time.sleep(sleep_for)
            # Spin until turn increments (safety)
            while True:

...
===============
Critical fixes:
- Immediate relocation to ANY adjacent connected plantation (not just escape targets).
- Build TWO plantations simultaneously after respawn (escape + extra) to get second builder faster.
- Use all available builders for both targets.
- More aggressive expansion from turn 1.
- Fixed turn skipping with hybrid sleep + spin.
"""
...
def main_loop():
    ...
    escape_targets = []          # list of adjacent cells we are building (max 2)
    last_cu_pos    = None
    last_upgrade_turn = -1

    logging.info("DatsSol bot v11 started.")
    ...
    while True:
        ...
        try:
            ...
            if last_turn != -1 and turn > last_turn + 1:
                logging.warning(f"  ⏭️ Skipped {turn - last_turn - 1} turn(s)!")
            last_turn = turn
            ...
            escape_targets = [t for t in escape_targets if t not in mountains and t not in enemy_pos_set and t not in connected]

            # Add new targets (always keep 2)
            if len(escape_targets) < 2:
                free_adj = []
                for nb in neighbors4(*main_pos):
                    ...
                    if nb not in p_map:
                        free_adj.append(nb)
                free_adj.sort(key=lambda p: (-cell_value(*p), p))
                for pos in free_adj:
                    if pos not in escape_targets:
                        escape_targets.append(pos)
                        logging.warning(f"  🔑 New escape target: {pos} (ЦУ={cu_cell_prog:.0f}%)")
                        if len(escape_targets) >= 2:
                            break
            ...
            # ── RELOCATION: immediate if any adjacent cell is already a connected plantation ──
            adjacent_plantations = [nb for nb in neighbors4(*main_pos) if nb in p_map and nb in connected]
            if adjacent_plantations:
                # Prefer enhanced cells
                target = max(adjacent_plantations, key=lambda p: cell_value(*p))
                relocate_cmd = [list(main_pos), list(target)]
                logging.warning(f"  🚀 Immediate relocate to existing plantation {main_pos}→{target}")
                escape_targets = []
            elif escape_targets:
                ready_target = next((t for t in escape_targets if t in connected), None)
                if ready_target:
                    relocate_cmd = [list(main_pos), list(ready_target)]
                    logging.warning(f"  🚨 RELOCATE ЦУ {main_pos}→{ready_target} (escape target ready)")
                    escape_targets = []

            # If not relocating, build escape targets + one extra expansion target
            if not relocate_cmd:
                # Build escape targets
                for target in escape_targets:
                    builders = sorted(
                        connected_plants,
                        key=lambda p: chebyshev(tuple(p["position"]), target)
                    )[:5]
                    assigned = 0
                    for builder in builders:
                        if cb.try_assign(tuple(builder["position"]), target, "escape-build"):
                            assigned += 1
                    if assigned > 0:
                        constr_map = {tuple(c["position"]): c.get("progress", 0) for c in constr_list}
                        prog_str = f" server_prog={constr_map[target]}hp" if target in constr_map else ""
                        logging.warning(
                            f"  🔨 Build escape {target}{prog_str} "
                            f"(ЦУ={cu_cell_prog:.0f}%, need {build_turns_needed} turns, {assigned} builders)"
                        )

                # Also build one extra expansion target (different from escape) to get second plantation faster
                frontier = get_frontier(connected, occupied, mountains, ar, map_w, map_h, beaver_positions)
                for target_pos, score in frontier:
                    if target_pos in escape_targets:
                        continue
                    builders = sorted(
                        connected_plants,
                    ...
                    for builder in builders:
                        if cb.try_assign(tuple(builder["position"]), target_pos, f"expand-extra({score:.0f})"):
                            logging.info(f"  🌱 Extra expand {target_pos} score={score:.0f}")
                            break
                    break  # only one extra build per turn
            ...
            # ── Hybrid sleep + spin for turn sync ───────────────────────
            elapsed = time.time() - loop_start
            sleep_for = max(0.0, next_turn_in - elapsed - 0.3)
            if sleep_for > 0.001:
                time.sleep(sleep_for)
            # Spin until turn increments
            while True:
                check_state = get_arena()
                if check_state.get("turnNo", 0) != turn:
                    break
                time.sleep(0.05)

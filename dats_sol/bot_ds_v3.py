...
- Start building escape target immediately after respawn (0% cell progress).
- Fix turn skipping by using more precise sleep and retry logic.
- Reset last_upgrade_turn on ЦУ respawn to avoid duplicate upgrades.
- Log skipped turns for monitoring.
- Increase escape build priority: if no other commands, always build escape.
- Adjust sleep calculation to account for network latency dynamically.
"""
...
def main_loop():
    ...
    logging.info("DatsSol bot v7 started.")
    ...
    while True:
        ...
        try:
            ...
            # Pick new escape target immediately (since CU_BUILD_NEIGHBOR_PCT = 0)
            ...
            # ── SEND ────────────────────────────────────────────────────
            cmds = cb.commands
            # Force escape build if nothing else and we have a target
            ...
            # ── Precise sleep with dynamic latency adjustment ────────────
            elapsed = time.time() - loop_start
            # Target wake-up 0.03s before next turn to allow for GET latency
            sleep_for = max(0.0, next_turn_in - elapsed - 0.03)
            if sleep_for > 0.001:
